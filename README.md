...
- **[Подробный гайд по генерации .conf файлов](info/CONFIG_GENERATION_GUIDE.md)** — пошаговая инструкция от установки WireGuard на сервере до отдачи готового конфига пользователю (с генерацией ключей, настройкой vk-turn-proxy-server, QR-кодами, troubleshooting).
- **[Анализ v1 vs v2](info/ANALYSIS_v1_vs_v2.md)** — почему proxy_v2 скрыт из UI.
- **[Детали интеграции TURN](info/TURN_INTEGRATION_DETAILS.md)** — техническая документация для разработчиков.
- **[Пример конфигурации](info/config_example.conf)** — шаблон .conf файла со всеми полями.
